...
g.attr(rankdir="TB", splines="spline", nodesep="0.5", ranksep="0.9", bgcolor="white",
       ...
       label="Java Pet Store — Migrated Architecture (Spring Boot 3.3.5 / Java 21)\\l"
             "8 services + embedded Artemis broker + shared libraries · "
...
# ── services ───────────────────────────────────────────────────────────
svc("store", "petstore-app-v1", ":8080 · storefront (hosts broker)",
    ["GET / /category /product /item /search", "cart add/set/update/delete",
     "GET/POST /checkout  → publish only", "POST /api/checkout", "embeds cart-lib · i18n en/ja/zh"],
    STORE)
...
svc("catalog", "catalog-service", ":8083 · catalog (multi-locale)",
    ...
    CAT, db_label="category/product/item")

svc("opc", "order-processing-service", ":8088 · Order Processing Center (opc.ear)",
    ["consumes PurchaseOrderQueue → persist", "auto-approve / PENDING", "consumes InvoiceTopic → COMPLETED",
     "facade: /api/orders approve|deny|status"],
    OPC, db_label="orders + status (authoritative)")
...
svc("inventory", "inventory-service", ":8085 · fulfilment (supplier.ear)",
    ["consumes ApprovedOrderQueue → reserve+ship", "publishes InvoiceTopic", "GET/POST /api/inventory restock (SUPPLIER)"],
    INV, db_label="inventory (pessimistic lock)")

svc("notify", "notification-service", ":8087 · customer emails",
    ["subscribes InvoiceTopic", "composes 'Order Shipped' email", "legacy MailInvoiceMDB"],
    NOTE)
...
g.node("broker",
    ...
    f'<BR/><FONT COLOR="white" POINT-SIZE="8">:61616 · embedded in petstore-app-v1</FONT></TD></TR>'
    f'<TR><TD BGCOLOR="{BROKER[0]}"><FONT POINT-SIZE="8.5">'
    f'PurchaseOrderQueue (queue)<BR/>ApprovedOrderQueue (queue)<BR/>InvoiceTopic (topic, fan-out)</FONT></TD></TR>'
...
lib("opcsdk", "order-processing-client", "OPC facade SDK")
...
http("admin", "opc", "list / approve / deny")
...
jms("store", "broker", "PurchaseOrder")
jms("broker", "opc", "PurchaseOrderQueue")
jms("opc", "broker", "OrderApproved")
jms("broker", "inventory", "ApprovedOrderQueue")
jms("inventory", "broker", "Invoice")
jms("broker", "opc", "InvoiceTopic")
jms("broker", "notify", "InvoiceTopic")
...
imp("store", "custsdk"); imp("store", "catsdk")
imp("opc", "msg"); imp("opc", "authclient"); imp("opc", "opcsdk")
