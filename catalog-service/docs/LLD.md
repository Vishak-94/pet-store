...
Generated by [`catalog-service_lld.py`](catalog-service_lld.py) (shared house style
in `../../docs/lld_style.py`). Regenerate with
`cd catalog-service/docs && python3 catalog-service_lld.py`.

- **Class diagram** — [`catalog-service_class.png`](catalog-service_class.png)
  ([svg](catalog-service_class.svg)): the full hexagonal layering — Web →
  `CatalogService` → the **two ports** (`CatalogRepository` browse +
  `CatalogSearchPort` search, ISP) → the **two `@Profile` adapters**
  (`JpaCatalogRepository` `!mongo`, `MongoCatalogRepository` `mongo`) — plus the
  framework-free domain, the JPA locale-split entities, and the reusable
  `catalog-service-client` SDK. The port/adapter seam and profile-swap are the
  visual centrepiece.
- **Schema diagram** — [`catalog-service_schema.png`](catalog-service_schema.png)
  ([svg](catalog-service_schema.svg)): the two persistence models that sit behind
  the same port — the **H2 locale-split ER** model (`base` + `_details` keyed by
  `(id, locale)`, with PK/FK) and the **MongoDB Option-C** document model (three
  collections, embedded per-locale `details` map, denormalized `categoryId` /
  `productName` on items).
...
- **The client SDK (`catalog-service-client`).** `CatalogServiceClient` (a
  `RestClient` wrapper), the `CatalogServiceEndpoints` path/param constants, and
  the `CatalogDtos` records are packaged as an importable jar. Any caller (the
  monolith storefront and other services) constructs `new CatalogServiceClient(baseUrl)`
  and calls typed methods — no URLs or JSON shapes leak into caller code. The
  **`app` module itself depends on the client jar**, so the controller's
  `@GetMapping`s reference the same `CatalogServiceEndpoints` constants the clients
  call — server and callers provably share one contract and cannot drift.
- **The shared LLD diagram library** (`../../docs/lld_style.py`) renders these
  diagrams in the same house style as every other module — reuse at the docs layer.
- **The framework-free domain** (`Category`, `Product`, `Item`, `Page`) is reused
  unchanged by *both* persistence adapters: `JpaCatalogRepository.toItem(...)` and
  `MongoCatalogRepository.toItem(...)` both build the identical `Item` VO, and
  `Page.EMPTY_PAGE` is the one canonical empty result across every miss path.
- **The ports** (`CatalogRepository`, `CatalogSearchPort`) are the reuse seam the
  `CatalogService` binds to; the service is written once against the interfaces and
  works with whichever adapter the active profile wires in.
- **The Mongo seed reuses the relational seed data** — `MongoCatalogSeeder` loads
  the exact ids/prices/translations of `resources/data.sql` (same `en_US`, `ja_JP`,
  `zh_CN`), so both stores return identical results.

### How it is extended safely

- **Swap the store behind the port (already demonstrated).** The whole reason there
  are *two* adapters is extensibility: `JpaCatalogRepository` (`@Profile("!mongo")`)
  and `MongoCatalogRepository` (`@Profile("mongo")`) both implement
  `CatalogRepository` **and** `CatalogSearchPort`; exactly one is active per profile,
  selected by `SPRING_PROFILES_ACTIVE=mongo`. Adding a *third* store (e.g. a cache or
  a different DB) is a new `@Profile` adapter class plus an `application.yml` profile
  document — **the domain, `CatalogService`, `CatalogApiController`, and the client
  SDK are untouched.**
- **Evolve search independently (the ISP payoff).** `CatalogSearchPort` is a
  separate, one-method interface precisely so keyword search can move to a dedicated
  engine (Elasticsearch/OpenSearch) or its own service without touching the browse
  contract or its adapters. A new search backend is a new class implementing only
  `CatalogSearchPort`, injected into `CatalogService` alongside the browse adapter.
  Within the JPA adapter, the dynamic-query concern is itself isolated in the
  `ItemSearchRepository` custom fragment (`ItemSearchRepositoryImpl`), so the search
  SQL can change without disturbing the derived Spring Data queries.
- **Add a field without breaking callers.** `CatalogDtos` are records; Jackson maps
  by component name, so adding a component is backward compatible — old callers
  ignore the new field, new callers read it. Removing/renaming is the breaking case
  (see `../CLAUDE.md`).
- **Add an endpoint.** Add a constant to `CatalogServiceEndpoints`, a `@GetMapping`
  in `CatalogApiController` referencing it, a port method + adapter impls, and a
  client method — the single-sourced-constant pattern keeps the two ends aligned.
