package com.petstore.warehouse.config;
 ...
 *
 * <p><b>Why method-aware:</b> retrying a POST/PUT (e.g. approve/deny) could
 * double-submit a non-idempotent side effect, so retry is applied to
 * {@code GET}/{@code HEAD}/{@code OPTIONS} only. The circuit breaker is applied to
 * ALL methods — it never re-executes a call, it just fails fast once a downstream is
 * clearly unhealthy, which is safe for writes too.
 */
public final class ResilientRestClient {
    ...
    private static ClientHttpRequestInterceptor resilienceInterceptor(CircuitBreaker breaker, Retry retry) {
